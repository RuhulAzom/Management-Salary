...
exports.AddEmployeeSalary = async (req, res) => {
    try {
        const { employee_id, start_date, end_date, member, salary_per_day } = req.body;

        if (!employee_id || !start_date || !end_date || !member || !salary_per_day) return res.status(404).json({
            status: 404,
            message: "Data is Not Valid!"
        })

        const data = await getDetailEmployeeDataObject2({ start_date, end_date, employee_id, salary_per_day })
...
exports.EditEmployeeSalaryById = async (req, res) => {
    try {
        ...
        //     message: "Data is not valid!"
        // })
        const { id, employee_id, start_date, end_date, member, salary_per_day } = req.body;

        if (!id || !employee_id || !start_date || !end_date || !member || !salary_per_day) return res.status(404).json({
            status: 404,
            message: "Data is Not Valid!"
        })
...
exports.GetSalaryByEmployeeId = async (req, res) => {
    try {
        ...
        const total_employee_salary = await prisma.employee_Salary.count({ where: { employee_id } })
...
const getDetailEmployeeDataObject2 = async ({ start_date, end_date, employee_id, salary_per_day }) => {
    let overtime;
    let permit;
    ...
    const differenceInDays = Math.floor(differenceInMilliseconds / (1000 * 60 * 60 * 24));
    const attendance = differenceInDays - permit.length < 0 ? 0 : differenceInDays - permit.length
