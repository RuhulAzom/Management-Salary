...
export default function AddEmployee() {
  ...
  const AddEmploye = async () => {
    ...
    try {
      const res = await axios.post(
        ...
        {
          name: inputData.name,
          first_enter: inputData.firstEnter,
      ...
      console.log(res);
      toast.success(`Succes To Add ${inputData.name} as Employee`, {
        duration: 3000,
      });
      setInputData({
        name: "",
  ...
  return (
    <div className="px-[1rem] md:px-[4rem] py-[2rem] bg-body">
      <LoadingPageWithText
        heading="Adding Employee...."
        loading={inputData.loading}
      />
      <Link
        to={"/employee"}
        className="flex gap-[1rem] justify-start items-center mb-[2rem] hover:bg-main-gray-border cursor-pointer select-none w-fit px-[.8rem] rounded-[1rem] duration-300"
      >
      ...
      <div className="rounded-sm bg-white shadow-xl">
        ...
        <form
          className="flex flex-col gap-[1.5rem] p-[1.5rem]"
          onSubmit={(e) => {
            AddEmploye();
            e.preventDefault();
          }}
        >
          ...
          <div className="flex flex-col gap-[.5rem]">
            ...
            <Select
              ...
              onValueChange={(value) => {
                if (value)
                  setInputData((prev) => ({ ...prev, branchId: value }));
              }}
            >
