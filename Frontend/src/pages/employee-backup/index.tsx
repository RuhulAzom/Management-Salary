...
export default function Employee() {
  ...
  const getEmployee = async (page: number) => {
    try {
      ...
      if (filter.active) {
        res = await axios.get(
          `${API_URL}/employee?start_date=${getDateForInput(
            filter.startDate
          )}&end_date=${getDateForInput(filter.endDate)}&page=${page}`,
          {
            headers: {
  ...
  const searchByType = async ({
    ...
    try {
      ...
      if (filter.active) {
        res = await axios.get(
          `${API_URL}/employee/search?${type}=${searchValue}&start_date=${getDateForInput(
            filter.startDate
          )}&end_date=${getDateForInput(filter.endDate)}&page=${page}`,
          {
            headers: {
        ...
        res = await axios.get(
          `${API_URL}/employee/search?${type}=${searchValue}&page=${page}`,
          {
            headers: {
              Authorization: `bearer ${token}`,
            },
  ...
  return (
    <div className="py-[2rem] px-[1rem] md:px-[4rem] flex flex-col gap-[2rem] bg-body">
      ...
      />
      <LoadingPageWithText
        heading={`Deleting "${modalDelete.title}"`}
        loading={loading.for === "delete" ? loading.active : false}
      />
      <LoadingPageWithText
        heading={`Loading Fetch...`}
        loading={loading.for === "fetch" ? loading.active : false}
      />
      ...
      <div className="flex items-center justify-between w-full">
        ...
        <Link
          to={"/employee/add"}
          className="md:hidden shrink-0 bg-main-purple text-main font-[500] px-[1.5rem] py-[.8rem] rounded-[1rem] hover:bg-main-purple-hover active:bg-main-purple duration-200 shadow-table-black"
        >
          Add Employee
        </Link>
      </div>
      ...
      <div className="flex w-full items-center justify-between gap-[1rem]">
        <div className="flex items-center gap-[2rem]">
          <form
            className="w-full xl:w-fit relative flex items-center text-main-gray-text"
            onSubmit={(e) => {
              e.preventDefault();
              if (searchValue.length === 0) {
          ...
          >
            ...
            <input
              type="text"
              placeholder="Search..."
              className="w-full xl:w-[400px] shadow-table-black outline-none bg-white  h-[42px] px-[3rem] rounded-[.8rem] border border-white focus:border-main-gray-border hover:border-main-gray-border duration-300"
              onChange={(e) => {
            ...
            {searchValue.length > 0 && (
              <i
                className="bx bx-x absolute right-4 text-[1.5rem] duration-300 hover:bg-main-gray-border rounded-[50%]"
                onClick={() => {
                  setSearchValue("");
                  setPage(1);
                }}
          ...
          <Select
            value={typeSearch}
            onValueChange={(value) =>
              value && setTypeSearch(value as "member" | "name")
            }
          >
          ...
          <div className="relative">
            <div
              className={cn(
                "shadow-table-black outline-none bg-white flex justify-center items-center gap-[.5rem] h-[42px] px-[1.5rem] rounded-[.8rem] border border-white focus:border-main-gray-border hover:border-main-gray-border duration-300 text-main-gray-text cursor-pointer",
            ...
            {filter.show && (
              <form
                className={cn(
                  "absolute z-[10] bg-white top-[100%] left-0 shadow-default-black p-[1rem] rounded-[.5rem] flex flex-col gap-[1rem]"
                )}
                onSubmit={(e) => {
                  e.preventDefault();
              ...
              >
                <div className="flex justify-between gap-[.5rem]">
                  <div className="input-poster flex flex-col gap-[.5rem]">
                    ...
                    <input
                      type="date"
                      className="px-[1rem] py-[.7rem] rounded-[.5rem] outline-none border border-main-gray-border w-full"
                ...
                <div className="w-full flex justify-end gap-[1rem]">
                  <div
                    className={cn(
                      "bg-main-purple text-main font-[500] px-[1.5rem] py-[.8rem] rounded-[1rem] hover:bg-main-purple-hover active:bg-main-purple duration-200 shadow-table-black cursor-pointer",
                      filter.startDate.length === 0 &&
                        "bg-main-gray-border2 text-main-gray-text/50 cursor-not-allowed hover:bg-main-gray-border2 active:bg-main-gray-border2",
                  ...
                  <button
                    className={cn(
                      "bg-main-purple text-main font-[500] px-[1.5rem] py-[.8rem] rounded-[1rem] hover:bg-main-purple-hover active:bg-main-purple duration-200 shadow-table-black",
                      filter.startDate.length === 0 &&
                        "bg-main-gray-border2 text-main-gray-text/50 cursor-not-allowed hover:bg-main-gray-border2 active:bg-main-gray-border2",
                      filter.endDate.length === 0 &&
                        "bg-main-gray-border2 text-main-gray-text/50 cursor-not-allowed hover:bg-main-gray-border2 active:bg-main-gray-border2"
            ...
            {filter.show && (
              <div
                className="fixed z-[5] bg-transparent w-full h-full top-0 left-0"
                onClick={() => {
        ...
        <Link
          to={"/employee/add"}
          className="hidden md:block shrink-0 bg-main-purple text-main font-[500] px-[1.5rem] py-[.8rem] rounded-[1rem] hover:bg-main-purple-hover active:bg-main-purple duration-200 shadow-table-black"
        >
          Add Employee
        </Link>
      </div>
        ...
        <table className="w-full shadow-table-black rounded-[.8rem]">
          <thead>
            <tr className="border-b border-main-gray-border">
              <th
                className={`select-text text-center rounded-tl-[.8rem] bg-white p-[1rem] font-[500] md:text-[1rem]`}
              >
                No
              </th>
              <th
                className={`select-text text-center bg-white p-[1rem] font-[500] md:text-[1rem]`}
              >
                Nama
              </th>
              <th
              ...
              <th
                className={`select-text text-center rounded-tr-[.8rem] bg-white p-[1rem] font-[500] md:text-[1rem]`}
              >
                Action
              </th>
            </tr>
          ...
          <tbody>
            {data?.map((item, i: number) => (
              <tr
                ...
                } border-main-gray-border`}
              >
                <td
                  className={`select-text text-center ${
                    i === data.length - 1 && "rounded-bl-[.8rem]"
                  } bg-white p-[1rem] md:text-[.9rem] font-[400] text-main-gray-text`}
                >
                  {page * 10 + (i + 1) - 10}
                </td>
                <td
                  className={`select-text text-center bg-white p-[1rem] md:text-[.9rem] font-[400] text-main-gray-text`}
                >
                  {item.name}
                </td>
                <td
                ...
                <td
                  className={`select-text text-center bg-white p-[1rem] md:text-[.9rem] font-[400] text-main-gray-text`}
                >
                  {`Rp. ${item.total_salary.toLocaleString("id-ID", {
                    style: "decimal",
                  })}`}
                </td>
                <td
                  className={`text-center ${
                    i === data.length - 1 && "rounded-br-[.8rem]"
                  } bg-white p-[1rem] md:text-[.9rem] font-[400] text-main-gray-text`}
                >
                  <div className="flex gap-2 w-full justify-center">
                    <button
                      className="bg-blue-100 text-blue-600 py-[.8rem] px-[1.5rem] font-[600] rounded-[1rem] hover:bg-blue-200 active:bg-blue-100"
                      onClick={() => {
                    ...
                    <button
                      className="bg-main-pink text-main-red py-[.8rem] px-[1rem] font-[600] rounded-[1rem] hover:bg-main-pink-hover active:bg-main-pink"
                      onClick={() => {
                        setModalDelete((prev) => ({
                          ...prev,
                          title: item.name,
                    ...
                    <Link
                      to={getDetailHref(item.id, item.first_enter)}
                      className="bg-orange-100 text-orange-600 py-[.8rem] px-[1rem] font-[600] rounded-[1rem] hover:bg-orange-200 active:bg-orange-100"
                      onClick={() => {
                        setModalDelete((prev) => ({
                          ...prev,
                          title: item.name,
      ...
      <div className="w-full flex justify-between">
        ...
        <div className="flex items-center gap-[1rem] bg-white rounded-[.8rem] overflow-hidden shadow-table-black">
          <button
            className="px-[1rem] py-[.8rem] text-main font-[500] bg-main-purple hover:bg-main-purple-hover duration-200 active:bg-main-purple"
            onClick={() => {
              if (page > 1) {
                setPage(page - 1);
              }
