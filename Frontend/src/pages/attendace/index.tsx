  ...
  const getAttendanceById = async (id: string, page: number) => {
    ...
    try {
      const res = await axios.get(
        `${API_URL}/attendance?employee_id=${id}&page=${page}`,
        {
          headers: {
            Authorization: `bearer ${token}`,
          },
              ...
              <table className="w-full">
                ...
                <tbody>
                  {attendanceData.map((item, index) => (
                    <tr>
                      <td className="p-[.5rem] text-center text-[.7rem] md:text-[1rem]">
                        {index + 1}
                      </td>
                      <td className="p-[.5rem] text-start text-[.7rem] md:text-[1rem]">
                        ...
                        <div className="w-full flex justify-center items-center gap-[1rem]">
                          ...
                          <button
                            ...
                            onClick={() => {
                              setModalDelete((prev) => ({
                                ...prev,
                                title: `Attendance in ${getDateString(
                                  item.date
                                )}`,
                                show: true,
                                id: item.id,
